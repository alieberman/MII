...
angular.module('mftApp')
  .controller('TransferIndexOutboundCtrl', ['$scope', '$rootScope', '$resource', 'Transfers', '$routeParams', '$route', '$location',
	...
	$scope.byteSize = function(bytes) {
	  ...
	  Transfers.query(function(res) {
		...
		for (var i = 0; i < res.results.length; i++) {
			if (res.results[i].DIRECTION == "out") {
		...
		for (var i = 1; i <= 365; i++) {
			...
			for (var j = 0; j < res.results.length; j++) {
				if (res.results[j].DIRECTION == "out") {
					var isSuccessfulTransfer = null;
					if (res.results[j].EVENTTYPE == "File Upload Ended") {
						isSuccessfulTransfer = true;
					}
					else if (res.results[j].EVENTTYPE == "File Upload Terminated") {
						isSuccessfulTransfer = false;
					}
